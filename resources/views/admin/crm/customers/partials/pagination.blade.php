@if($paginator->total() > 0)
    @php
        ...
        $selectedPerPage = (int) request('per_page', 15);
    ...
    <div class="crm-leads-pagination">
        <div class="crm-leads-pagination__meta">
            <div class="crm-leads-pagination__summary">
                ...
                <span class="crm-leads-pagination__summary-total">of {{ number_format($paginator->total()) }} customers</span>
            </div>

            <form method="GET" action="{{ route('admin.crm.customers.index') }}" class="crm-leads-pagination__per-page">
                @foreach(request()->except(['page', 'per_page']) as $key => $value)
                    ...
                    @endif
                @endforeach
                <label for="crm-customers-per-page" class="crm-leads-pagination__per-page-label">
                    <iconify-icon icon="solar:layers-minimalistic-linear" aria-hidden="true"></iconify-icon>
                    <span>Per page</span>
                </label>
                <select id="crm-customers-per-page" name="per_page" class="crm-leads-pagination__per-page-select" onchange="this.form.submit()">
        ...
        @if($paginator->hasPages())
            <nav class="crm-leads-pagination__nav" aria-label="Customers pagination">
